  ...
  lis
    .forEach(li => {
      ...
      if (['expediente', 'estado', 'ultima_actualizacion'].includes(key)) {
        ...
        text = getNodesByXPath('div/div/a', li)
          .map(a => {
        ...
        .map(node => [...node, ...Array.from({length: 4 - node.length}, (_, i) => '')])
        .map(row => ({
          fecha: row[0],
          camara: row[1],
          fiscal: row[2],
        ...
        const involved = getNodesByXPath('div/div/div/ul', li)
          ...
          .map(row => {
            ...
            const [involved, rest] = li.innerText.split('VER LETRADOS')
            const layers = getNodesByXPath('div/div[@class="item"]', li)
              .map(div => div.textContent)

            if (layers.length) {
              return layers.map(layer => [relation, involved.trim(), layer.trim()])
            } else {
